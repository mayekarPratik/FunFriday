...
export const HostDayDisplay: React.FC = () => {
  ...
  // Sync 5-minute countdown timer
  useEffect(() => {
    if (!gameState?.timer_ends_at) return;

    const interval = setInterval(() => {
      const remaining = Math.max(0, gameState.timer_ends_at! - Date.now());
  ...
  }, [gameState?.timer_ends_at, tallyDayVotes]);
  ...
  return (
    <div className="w-full max-w-5xl mx-auto flex flex-col items-center text-center gap-8 py-4">
      ...
      <div className="wope-card p-8 sm:p-10 w-full flex flex-col items-center relative overflow-hidden shadow-2xl">
        ...
        {/* Voting Tally Progress & Host Action */}
          ...
          <div className="flex items-center gap-3 text-left">
            ...
            <div>
              <p className="text-sm font-bold text-[#F8FAFC]">
                {totalVotesCast} of {livingPlayers.length} Votes Submitted
              </p>
              <p className="text-xs text-[#94A3B8]">
                {totalVotesCast === livingPlayers.length
                  ? 'All votes in! Tallying automatically...'
                  : 'Waiting for players to finalize their vote...'}
              </p>
            </div>
          </div>
      ...
      <div className="w-full flex flex-col gap-4 text-left">
        <div className="flex items-center justify-between">
          <h3 className="text-base font-bold text-[#F8FAFC] flex items-center gap-2">
            ...
            Living Townspeople ({livingPlayers.length})
          </h3>
          ...
          {livingPlayers.map((player) => {
            ...
            return (
              <div
                ...
                className="wope-card p-3.5 flex items-center justify-between gap-2"
              >
                <div className="flex items-center gap-2.5 truncate">
                  <div className="w-8 h-8 rounded-full bg-[#1E293B] border border-[#1F2430] flex items-center justify-center text-xs font-bold text-[#F8FAFC] shrink-0">
